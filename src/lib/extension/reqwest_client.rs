...
impl ClientBuilder {
    ...
    pub fn get_client_with_auth(&mut self) -> RequestBuilder {
        ...
        if let Some(auth) = &self.bearer_auth {
            return request_client.bearer_auth(auth)
        } else if let Some(auth) = &self.basic_auth {
            return request_client.basic_auth(auth.username.to_string(), auth.password.clone());
        } else {
    ...
    ///
    /// Send the request, get the response bytes or `'Errors'` on fail
    /// 
    pub async fn send_request(&mut self) -> Result<Vec<u8>, Errors> {
